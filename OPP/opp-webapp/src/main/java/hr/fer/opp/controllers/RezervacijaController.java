...
public class RezervacijaController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * int apartmanID = 1; Rezervacija rezervacija = new Rezervacija();
		 * rezervacija.setApartman(DAOProvider.getDAO().getApartmanFor(
		 * apartmanID));
		 * rezervacija.setGost(DAOProvider.getDAO().getGostFor(1));
		 * rezervacija.setKorisnik(DAOProvider.getDAO().getKorisnikFor("3"));
		 * rezervacija.setDatumRezervacije(new GregorianCalendar(2016,
		 * Calendar.MAY, 1).getTime()); rezervacija.setRezerviranoOd(new
		 * GregorianCalendar(2016, Calendar.JUNE, 1).getTime());
		 * rezervacija.setRezerviranoDo(new GregorianCalendar(2016,
		 * Calendar.JULY, 15).getTime()); rezervacija.setParking(true);
		 * rezervacija.setInternet(true); rezervacija.setSatelitskaTV(true);
		 * Korisnik k = (Korisnik) req.getSession().getAttribute("korisnik");
		 * Korisnik korisnik =
		 * DAOProvider.getDAO().getKorisnikFor(k.getKorisnikID());
		...
		List<Rezervacija> rezervacije = DAOProvider.getDAO()
				.getReservationsFor(DAOProvider.getDAO().getApartmanFor(apartmanID));
	...
	private void rezerviraj(HttpServletRequest req, HttpServletResponse resp)
		...
		List<Date> slobodniDani = getList(
		...
		try {
			broj = Integer.parseInt(odrasi) + Integer.parseInt(godina0_1) + Integer.parseInt(godina2_7)
					+ Integer.parseInt(godina8_14);
		} catch (Exception e) {
		...
		if (index >= 0 && slobodniDani.get(index).equals(DateUtils.asDate(startRezervirano.minusDays(3)))) {
			...
			if (!(index > 0 && slobodniDani.get(index).equals(DateUtils.asDate(startRezervirano.minusDays(21))))) {
				List<Rezervacija> rezervacije = getRezervacijeFor(dateOd, dateDo, slobodniDani, opis, apartmanID,
						parking, internet, satelitskaTV, gost);
				;
				req.setAttribute("rezervacije", rezervacije);
				req.getSession().setAttribute("rezervacije", rezervacije);
				req.getServletContext().getRequestDispatcher("/WEB-INF/JSP/popisPredlozenihRezervacija.jsp")
						.forward(req, resp);
		...
		Korisnik korisnik = DAOProvider.getDAO()
				.getKorisnikFor(((Korisnik) req.getSession().getAttribute("korisnik")).getKorisnikID());
	...
	private void spremiRezervaciju(HttpServletRequest req, HttpServletResponse resp, Korisnik korisnik,
			Rezervacija rezervacija) throws IOException {
	...
	private List<Rezervacija> getRezervacijeFor(Date dateOd, Date dateDo, List<Date> slobodniDani, OpisApartmana opis,
			Integer apartmanID, boolean parking, boolean internet, boolean satelitskaTV, Gost gost) {
		...
		for (LocalDate date = startRezervirano; slobodniDani.get(index)
				.equals(DateUtils.asDate(date)); date = date.minusDays(1)) {
			index--;
			if (index < 0)
				break;
		}
		...
		for (Apartman apartman : DAOProvider.getDAO().getAllApartman()) {
			if (apartman.getApartmanID() == apartmanID)
				continue;
			List<Date> slobodni = getList(DAOProvider.getDAO().getReservationsFor(apartman));
	...
	private void errorRezervacija(HttpServletRequest req, HttpServletResponse resp, String message)
